...
class Batch(CNN):
    def __init__(self, img_path, char_set, model_save_dir, total):
        ...
        # 初始化变量
        super(Batch, self).__init__(image_height, image_width, len(label), char_set, model_save_dir)
        self.total = total
    ...
    def gen_captcha_text_image(self):
        ...
        if '.DS_Store' in img_file:
            ...
            # binarization_img = self.binarization(captcha_image)
            # clear_noise = self.clear_noise(binarization_img, 140, 3, 1)
    ...
    def test_batch(self):
        ...
        with tf.Session() as sess:
            ...
            for i in range(total):
                ...
                if test_text == p_text:
                    count.append(test_text)
                    right += 1
                else:
                    pass
            e = time.time()
        print("识别正确的验证码为：%s" % count)
...
def main():
    ...
    total = 100
    tb = Batch(test_image_dir, char_set, model_save_dir, total)
    tb.test_batch()
