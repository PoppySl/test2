...
def recognize_captcha(remote_url, rec_times, save_path, image_suffix):
    ...
    for index in range(rec_times):
        ...
        while True:
            try:
                response = requests.request(
                    "GET", remote_url, headers=headers, timeout=6)
        ...
        print(
            "【{}】 index:{} 耗时：{}ms 预测结果：{}".format(
                now_time, index, int(
                    (e - s) * 1000), predict_text))
...
def main():
    ...
    rec_times = 1
    result = recognize_captcha(remote_url, rec_times, save_path, image_suffix)
    return result
